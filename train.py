...
def eval(model, iterator, f, criterion, binary_criterion):
    ...
    with open(f, 'w') as fout:
        ...
        if num_task == 1:
            for words, is_heads, tags[0], y_hats[0] in zip(Words, Is_heads, f(*Tags), f(*Y_hats)):
                for i in range(num_task):
            ...
            true_neg = 0
            true_pos = 0
            for words, is_heads, tags[0], tags[1], y_hats[0], y_hats[1] in zip(Words, Is_heads, f(*Tags), f(*Y_hats)):
...
if __name__=="__main__":
    ...
    binary_criterion = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([3932/14263]))
    ...
    for epoch in range(1, hp.n_epochs+1):
        ...
        first_str = '[{{}:>{}}/{{}:>{}}]     '.format(epoch, epoch_len, hp.n_epochs, epoch_len)
        print_msg = (first_str +
    ...
    print_msg = ('test_precision: {:.5f} '.format(precision) +
                 'test_recall: {:.5f} '.format(recall) +
